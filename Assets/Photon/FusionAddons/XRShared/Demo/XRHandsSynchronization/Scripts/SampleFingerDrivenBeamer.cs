...
namespace Fusion.Addons.XRHandsSync.Demo
{
    public class SampleFingerDrivenBeamer : MonoBehaviour
    {
        ...
        private void Awake()
        {
            rigPart = GetComponentInParent<IHardwareHand>();
            rig = GetComponentInParent<IHardwareRig>();
            beamer = GetComponentInParent<RayBeamer>();
            if(beamer == null)
            {
                beamer = GetComponentInChildren<RayBeamer>();
            }
            if (rigPart == null || rig == null)
            ...
            var origin = beamer.origin != null ? beamer.origin : beamer.transform;
            rayDefaultPose = new Pose(
                rigPart.transform.InverseTransformPoint(origin.position),
                Quaternion.Inverse(rigPart.transform.rotation) * origin.rotation
            );
        }
        ...
        private void UpdatedHands(XRHandSubsystem subsystem, XRHandSubsystem.UpdateSuccessFlags succesFlags, XRHandSubsystem.UpdateType updateType)
        {
            ...
            switch (updateType)
            {
                case XRHandSubsystem.UpdateType.Dynamic:
                    ...
                    XRHand hand;
                    if (rigPart.Side == RigPartSide.Left)
                    {
                    ...
                    if (handavailable)
                    {
                        ...
                        if (indexTipAvailable && indexDistalAvailable && indexIntermediatePoseAvailable && indexProximalAvailable && thumbDistalAvailable && thumbProximalAvailable && indexMetacarpalAvailable && thumbMetacarpalAvailable)
                        {
                            ...
                            if (thumbPoseOk && (indexPoseOk || wasActive))
                            {
                                ...
                                actionDetected = true;
                                beamerPose = new Pose(indexProximalPose.position + indexMetacarpalPose.rotation * Vector3.up * 0.02f, indexMetacarpalPose.rotation);
                            }
                        }
                    }
                    else
                    ...
                    if (isBeamActive)
                    {
                        ...
                        beamer.origin.position = rig.transform.TransformPoint(beamerPose.position);
                        beamer.origin.rotation = rig.transform.rotation * beamerPose.rotation;
                    }
                    else
                    {
                        beamer.isRayEnabled = false;
                        // Pose was captured relatively to the hand
                        beamer.origin.position = rig.transform.TransformPoint(rayDefaultPose.position);
                        beamer.origin.rotation = rig.transform.rotation * rayDefaultPose.rotation;
                    }
