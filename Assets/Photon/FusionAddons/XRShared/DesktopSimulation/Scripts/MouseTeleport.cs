...
namespace Fusion.XR.Shared.Desktop
{
    ...
    public class MouseTeleport : MonoBehaviour
    {
        ...
        public IHardwareRigPart rigPart;
        public IOverridableGrabbingProvider grabbingProvider;
        public IHardwareRigPart beamerHand;
        ...
        protected virtual void Awake()
        {
            ...
            if (rig == null) rig = GetComponentInParent<IHardwareRig>();
            DetectSimulatedRigPart();
        }

        void StoreDefaultPosition()
        {
            defaultHandPosition = Head.InverseTransformPoint(rigPart.transform.position);
            defaultHandRotation = Quaternion.Inverse(Head.rotation) * rigPart.transform.rotation;
        }

        void DetectSimulatedRigPart()
        {
            if (grabbingProvider != null && beamerHand != null) return;
            foreach(var availableRigPart in rig.RigParts)
            {
                // grab and teleport are done with left hand
                if (availableRigPart is IHardwareController controller && controller.Side == RigPartSide.Left)
                {
                    rigPart = controller;
                    StoreDefaultPosition();
                    if (controller is IOverridableGrabbingProvider grabbingProvider)
                    {
                        this.grabbingProvider = grabbingProvider;
                    }
                    beamerHand = controller;
                    rayBeamer = rigPart.gameObject.GetComponentInChildren<RayBeamer>();
                }
            }
        }
                ...
                if (Physics.Raycast(mouseRay, out RaycastHit hit, 40f))
                {
                    ...
                    if (grabbed == null)
                    {
                        ...
                        if (grabbableObject != null)
                        {
                            ...
                            grabbingProvider.gameObject.transform.position = hit.point;
                            grabbingProvider.OverrideGrabbing(true);
        ...
        void CheckUngrab()
        {
            ...
            if (!Mouse.current.leftButton.wasReleasedThisFrame && !Mouse.current.leftButton.isPressed)
            {
                ...
                {
                    grabbed = null;
                    grabbingProvider.OverrideGrabbing(false);
                }
            }
#endif
        }
        ...
        protected virtual void Update()
        {
            ...
            if (grabHandDistance != 0)
            {
                ...
                rigPart.transform.position = mouseRay.origin + mouseRay.direction * grabHandDistance;
            }

            if (!grabberHandPositionHandled)
            {
                var leftHandPosition = Head.TransformPoint(defaultHandPosition) + mouseRay.direction * HAND_RANGE;
                if ((leftHandPosition - rigPart.transform.position).sqrMagnitude < 1f)
                {
                    leftHandPosition = Vector3.Lerp(rigPart.transform.position, leftHandPosition, 30f * Time.deltaTime);
                }
                rigPart.transform.position = leftHandPosition;
            }
            if (!beamerRotationHandled)
            {
                rigPart.transform.LookAt(mouseRay.origin + mouseRay.direction * 100f);
                rigPart.transform.Rotate(-40, 0, 0);
            }
#endif
        }
    }
