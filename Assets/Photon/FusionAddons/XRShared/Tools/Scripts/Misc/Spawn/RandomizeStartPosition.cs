...
namespace Fusion.XR.Shared
{
    public class RandomizeStartPosition : MonoBehaviour
    {
        ...
        private void Awake()
        {
            if (GetComponent<ISimulatedRigComponent>() != null)
            {
                // Not in VR
            ...
            {
                // VR
                var headset = GetComponentInChildren<IHardwareHeadset>();
                if (headset != null && childHeadsetToAlign == null) childHeadsetToAlign = headset.transform;
            }
        ...
        async void AlignHeadsetInVR()
        {
            ...
            while (HeadsetDetected == false && Time.time < waitEnd)
            {
                await AsyncTask.Delay(100);
                if (childHeadsetToAlign == null)
                {
                    return;
                }
            }
